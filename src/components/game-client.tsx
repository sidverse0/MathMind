...
import { Coins, Plus, Minus, X, Divide, BrainCircuit, Trophy, Timer, CheckCircle, XCircle, Gamepad2, Sparkles } from 'lucide-react';
...
export function GameClient() {
  ...
  const renderPhase = () => {
    ...
    return (
      <div className="relative w-full min-h-[350px] flex items-center justify-center overflow-hidden">
            ...
            >
                ...
                <div className="w-full text-center">
                    ...
                    {(Object.keys(categoryIcons) as MathCategory[]).map((cat) => (
                        ...
                        size="lg"
                        className={cn("h-28 text-lg flex-col gap-2 transition-all duration-300 transform hover:scale-105", state.category === cat ? 'scale-105 border-primary border-2 shadow-lg shadow-primary/20' : 'hover:border-primary/50')}
                ...
                <div className="text-center">
                    ...
                    <div className="text-6xl font-bold tracking-widest text-primary my-8 bg-secondary/50 p-4 rounded-lg">
                ...
                <div className="text-center flex flex-col items-center justify-center">
                    {state.feedback === 'correct' && <motion.div initial={{scale:0}} animate={{scale:1}} transition={{type: 'spring'}}><CheckCircle className="w-24 h-24 text-green-500 mb-4" /></motion.div>}
                    {state.feedback === 'incorrect' && <motion.div initial={{scale:0}} animate={{scale:1}} transition={{type: 'spring'}}><XCircle className="w-24 h-24 text-red-500 mb-4" /></motion.div>}
                    {state.feedback === 'timeup' && <motion.div initial={{scale:0}} animate={{scale:1}} transition={{type: 'spring'}}><Timer className="w-24 h-24 text-yellow-500 mb-4" /></motion.div>}
        ...
        <CardContent>
            {accuracyData.length > 1 ? (
                ...
                <div className="h-[250px] flex items-center justify-center text-muted-foreground text-center p-4">
                    Play a few rounds to see your progress chart!
                </div>
            )}
        </CardContent>
      </Card>
